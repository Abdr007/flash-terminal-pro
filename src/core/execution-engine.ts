...
export class ExecutionEngine implements IExecutionEngine {
  ...
  private handleHelp(): TxResult {
    // Exact flash-terminal help format: 32-char column, same categories, same order
    const C = 32; // column width matching flash-terminal
    const cmd = (s: string) => accentBold(s.padEnd(C));
